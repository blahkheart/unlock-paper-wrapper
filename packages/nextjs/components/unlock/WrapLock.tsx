import { useState } from "react";
import { ArrowSmallRightIcon, ClipboardDocumentIcon, XMarkIcon } from "@heroicons/react/24/outline";
import { lockWrapperABI } from "~~/abi/unlockPaperWrapperABI";
import { useScaffoldContractWrite } from "~~/hooks/scaffold-eth";
import { notification } from "~~/utils/scaffold-eth";

export const WrapLock = () => {
  const [visible, setVisible] = useState(true);
  const [lockAddress, setLockAddress] = useState("");

  const { writeAsync, isLoading } = useScaffoldContractWrite({
    contractName: "UnlockPaperWrapperFactory",
    functionName: "createWrapper",
    args: [lockAddress],
    onBlockConfirmation: txnReceipt => {
      console.log("📦 Transaction blockHash", txnReceipt.blockHash);
    },
  });
  const handleCopySignerABI = () => {
    if (lockWrapperABI) {
      const abiString = JSON.stringify(lockWrapperABI);
      navigator.clipboard.writeText(abiString);
      notification.success("Copied ABI");
    } else {
      notification.error("Nothing to copy");
      return;
    }
  };
  return (
    <div className="flex bg-base-300 relative pb-10">
      <div className="flex flex-col w-full mx-5 sm:mx-8 2xl:mx-20">
        <div className={`mt-10 flex gap-2 ${visible ? "" : "hidden"} max-w-2xl`}>
          <div className="flex gap-5 bg-base-200 bg-opacity-80 z-0 p-7 rounded-2xl shadow-lg">
            <span className="text-3xl">👋🏻</span>
            <div>
              <div>
                This tool lets you create a wrapper for your <strong>Unlock Protocol</strong> Lock smart contracts to
                enable compatibility with <strong>Paper.xyz</strong>. Begin by entering your lock address into the input
                below and click the send button, then copy the deployed wrapper address and ABI for use in your
                paper.xyz dashboard when <strong>registering a contract</strong>. That&apos;s it! Now you can create
                custom Paper checkouts for your locks. Have fun trying it out!
              </div>
            </div>
          </div>
          <button
            className="btn btn-circle btn-ghost h-6 w-6 bg-base-200 bg-opacity-80 z-0 min-h-0 drop-shadow-md"
            onClick={() => setVisible(false)}
          >
            <XMarkIcon className="h-4 w-4" />
          </button>
        </div>

        <div className="flex flex-col mt-6 px-7 py-8 bg-base-200 opacity-80 rounded-2xl shadow-lg border-2 border-primary">
          <span className="text-4xl sm:text-5xl text-center">Wrap Your Lock</span>

          <div className="mt-8 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-5">
            <input
              type="text"
              placeholder="Enter lock address"
              className="input font-bai-jamjuree w-full px-5  border border-primary text-base min-w-350"
              onChange={e => setLockAddress(e.target.value)}
            />
            <div className="flex rounded-full border border-primary p-1 flex-shrink-0">
              <div className="flex rounded-full border-2 border-primary p-1">
                <button
                  className={`btn btn-primary rounded-full capitalize font-normal font-white w-24 flex items-center gap-1 hover:gap-2 transition-all tracking-widest ${
                    isLoading ? "loading" : ""
                  }`}
                  onClick={() => writeAsync()}
                >
                  {!isLoading && (
                    <>
                      Send <ArrowSmallRightIcon className="w-3 h-3 mt-0.5" />
                    </>
                  )}
                </button>
              </div>
            </div>
          </div>

          <div className="mt-4 flex gap-2 items-start items-center">
            <span className="text-sm leading-tight">Copy ABI:</span>

            <button
              className="btn rounded-lg btn-sm mr-3"
              onClick={() => {
                handleCopySignerABI();
              }}
            >
              <ClipboardDocumentIcon className="h-3 w-3" />
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};
